...
public class StepSoundScript : MonoBehaviour
{
    ...
    public void DoStep()
    {
        if (Floortypes != null)
        {
            ...
            stepPlayer.volume = Stepsounds[stepIndex].volume * (1f + UnityEngine.Random.Range(-Stepsounds[stepIndex].volumeVariance / 2f, Stepsounds[stepIndex].volumeVariance / 2f));
            stepPlayer.pitch = Stepsounds[stepIndex].pitch * (1f + UnityEngine.Random.Range(-Stepsounds[stepIndex].pitchVariance / 2f, Stepsounds[stepIndex].pitchVariance / 2f));
